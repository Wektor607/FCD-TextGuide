...
class Preprocess:
    ...
    def __init__(
        ...
        self._subject_ids = site_codes #None
    ...
    def correct_sulc_freesurfer(self, vals, mask=None):
        """this function normalized sulcul feature in cm when values are in mm (depending on Freesurfer version used)"""
        if mask is None:
            if np.mean(vals, axis=0) > 0.2:
                vals = vals / 10
    ...
    def plot_subject_features(self, features_to_plot):
        ...
        for subj_id in self.subject_ids:
            ...
            os.makedirs(os.path.join(BASE_PATH, f'{subj.site_code}', "images"), exist_ok=True)
            ...
            for hemi in hemis:
                ...
                mpt.plot_single_subject(
                    ...
                    out_filename=os.path.join(BASE_PATH, f'{subj.site_code}', "images", f"qc_features_{subj_id}_{hemi}.jpeg"),
                )
    ...
    def get_data_preprocessed(
        ...
        for hemi in ("lh", "rh"):
            ...
            if only_lesion:
                ...
                if lobes and only_features == False:
    ...
    def get_combat_new_site_parameters(
        ...
        site_combat_path = os.path.join(self.data_dir,f'{site_code}','distributed_combat')
