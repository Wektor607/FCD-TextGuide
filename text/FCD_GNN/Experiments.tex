...
The experimental variants differ only in (i) whether the \texttt{GuideDecoder} is inserted before the upsampling path and (ii) the type of atlas-derived textual input used. 
The pretrained \texttt{\texttt{Vision--only}} model (described below) is used to initialize the decoder for all variants except the MELD-only baseline, which is trained from scratch.


We consider the following configurations:

\begin{itemize}
    \item \textbf{MELD.} The pretrained \ac{meld} model was evaluated on the test set without any additional training or fine-tuning and is used as the baseline (see Fig.~\ref{fig:meld_arch}).
    ...
    The text branch and the self--/cross--attention modules of the \texttt{GuideDecoder} are disabled, while the geometry--based upsampling components are retained (see Fig.~\ref{fig:exp1_arch}).
    ...
    One \texttt{GuideDecoder} block is inserted at each decoder level (D6--D1), i.e., one block per upsampling stage, to refine the features using self--attention before the subsequent upsampling step (see Fig.~\ref{fig:exp2_arch}).
    ...
    In contrast to \texttt{Vision--only+SA}, the full \texttt{GuideDecoder} is enabled, i.e., self--attention on image tokens and cross--attention between image and text embeddings are applied (see Fig.~\ref{fig:basic_model}).
...
Throughout this section, we do not focus on the median PPV$_{\text{clusters}}$, which is the primary metric reported in the original MELD study.
...
Each figure shows (from left to right) the ground-truth lesion mask, the prediction of the baseline MELD model, and the prediction of the proposed \texttt{Vision+LM} model\footnote{ Throughout this section, \texttt{Vision+LM} corresponds to Exp3.} with PubMedBERT using the \texttt{hemi+lobe} prompt.
...
% \clearpage
\section{Mixed Text}
This section investigates training with \textit{mixed text prompts} and the effect of a short decoder warm--up phase.
...
All experiments in this section use \textbf{RadBERT} as the text encoder.
...
A possible explanation for this behavior is discussed in Section~\ref{chapter:Discussion}.
...
Based on the observations from the previous sections, we proceed with a set of follow--up
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Vision+LM\_hemi+lobe\_regions} provides the most balanced performance and achieves the 
        ...
        \item \textbf{Vision+LM\_hemi+lobe+PubMedBERT} achieves the highest \textbf{Sensitivity} and \textbf{Specificity} on both cohorts. 
        The concise prompt without irrelevant context, combined with PubMedBERT’s domain-specific pretraining, 
        improves detection performance while only slightly reducing segmentation metrics (Dice/IoU/PPV).

        \item \textbf{Vision+LM\_full\_desc} underperforms relative to the hemi+lobe variants, suggesting that 
    ...
    \item From the \textbf{Linking MELD to GNN} chapter, we conclude that adding a GNN block that aggregates information from neighboring nodes 
    helps the model accumulate contextual information more effectively, leading to improved performance and higher sensitivity.
    However, aggregating information across deeper GNN layers introduces an oversmoothing effect, which causes node representations 
    to become less discriminative. As a result, \textbf{Specificity} and other segmentation metrics degrade sharply.
    Consequently, the benefit of this GNN block is most pronounced for large and sparse graphs, as observed in our experiments,
    while deeper aggregation is detrimental in denser graph settings. 
    In future work, integrating this block into the final architecture in a controlled manner may further enhance results.
