...
class MeldCohort:
    ...
    def _site_hdf5(self, site_code, group, write=False, hdf5_file_root=None, feature=None, harmo_code=None):
        ...
        if feature is not None and "combat" in feature:
...
class MeldSubject:
    ...
    @property
    def group(self):
        group = "patient"# self.get_demographic_features('Group')
    ...
    @property
    def site_code(self):
        site_code = self.subject_id #self.get_demographic_features('Harmo code')
        return site_code
    ...
    def get_lesion_hemisphere(self):
        ...
        with hdf5_file as f:
            ...
            if surf_dir_lh_path in f:
                surf_dir_lh = f[surf_dir_lh_path]
                if ".on_lh.lesion.mgh" in surf_dir_lh.keys():
                    return "lh"

            if surf_dir_rh_path in f:
                surf_dir_rh = f[surf_dir_rh_path]
                if ".on_lh.lesion.mgh" in surf_dir_rh.keys():
                    return "rh"
        return None
